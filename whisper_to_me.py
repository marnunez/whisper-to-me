...
class WhisperToMe:
    ...
    def __init__(self, trigger_key=keyboard.Key.scroll_lock, model_size="large-v3", 
                 device="cuda", audio_device=None, debug=False):
        """
        ...
        self.speech_processor = SpeechProcessor(model_size=model_size, device=device)
        ...
        if key == self.trigger_key and self.is_recording:
            ...
            if audio_data is not None and len(audio_data) > 0:
                ...
                if self.debug:
                    ...
                    debug_filename = f"debug_recording_{self.recording_counter:03d}.wav"
...
Examples:
  ...
  %(prog)s --list-devices               # Show available audio devices
    ...
    parser.add_argument("--device", default="cuda", 
                       help="Processing device (cpu, cuda)")
    parser.add_argument("--key", default="scroll_lock", 
    ...
    parser.add_argument("--debug", action="store_true", 
                       help="Save recorded audio files for debugging")
    ...
    if args.list_devices:
        ...
        for device in devices:
            default_mark = " (default)" if default and device['id'] == default['id'] else ""
